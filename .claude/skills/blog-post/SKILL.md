...
Installed at `.claude/skills/blog-post/` so `/blog-post` invokes it.
Posts go to `blog/` at the repo root. Kit files live in `agent-kit/`;
...
4. Save to `blog/YYYY-MM-DD-<slug>.md` with frontmatter:
   `title`, `date`, `tags`.
