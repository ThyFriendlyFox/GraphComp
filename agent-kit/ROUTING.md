...
governs which situation. `SECURITY.md`, `CONTRIBUTING.md` and
`CHANGELOG.md` live at the repo root, where GitHub finds them. Do not improvise a process that a file below
already defines.
...
|---|---|---|
| First time this kit is in this repo (placeholders like `{{THIS}}` still exist anywhere in the kit, outside the reference table in `SETUP.md`) | `SETUP.md` | One-time tailoring. Do this before anything else. |
